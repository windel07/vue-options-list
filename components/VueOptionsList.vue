<template>
	<div>
		<div
			class="tw-mb-4 tw-flex tw-justify-between tw-items-center tw-text-sm"
		>
...
export default {
	...
	computed: {
		...
		paginatedList() {
			...
			const endIndex =
				startIndex + Math.floor(this.listScrollHeight / 25);
		...
		optionsList: {
			...
			handler(newValue) {
				if (newValue.length) {
					this.$nextTick(() => {
						setTimeout(() => {
							this.listScrollHeight =
								this.$refs.listScroller.offsetHeight;
							this.listScrollMax =
